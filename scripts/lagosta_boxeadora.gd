...
func _on_lagosta_colisao_body_entered(body: Node2D) -> void:
...
func _on_lagosta_colisao_body_exited(body: Node2D) -> void:
...
func _loop_ataque_continuo() -> void:
	while atacando and vida > 0 and inimigo_em_contato:
		if inimigo_em_contato and inimigo_em_contato.has_method("receber_dano"):
			anim.play("lagosta_attack")
			
			inimigo_em_contato.receber_dano(dano)
