...
var speed: int = 7
var inmgGarrafa_pos = Vector2i(0, 0)
...
func _ready():
	...
	anim.play('garrafa_walk')
	
func atacar(alvo):
		anim.play("garrafa_attack")
...
func _process(delta):
	tempoAttack += delta
	inmgGarrafa_pos = mapa.tilemap_layer.local_to_map(position)
	
	var celula_esquerda = Vector2i(inmgGarrafa_pos.x, inmgGarrafa_pos.y)
