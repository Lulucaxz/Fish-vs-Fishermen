...
var speed: int = 4
var inmgBarril_pos = Vector2i(0, 0)
...
func _ready():
	...
	anim.play('barril_walk')
	
func atacar(alvo):
		anim.play("barril_attack")
...
func _process(delta):
	tempoAttack += delta
	inmgBarril_pos = mapa.tilemap_layer.local_to_map(position)
	
	var celula_esquerda = Vector2i(inmgBarril_pos.x, inmgBarril_pos.y)
